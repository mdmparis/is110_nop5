...
#SBATCH -J nop5-c-ter_round-1
#SBATCH -o log_hmmsearch2.out
#SBATCH -e log_hmmsearch2.err

file_name="nop5-c-ter_round-1"
hmm_profile="nop5_round-1_90-90_rep_seq_kingdom=euk-arch_cured_nop-dom_cured.hmm"
min_len=200
max_len=1000
...
while read ID s e l qc tc; do seqkit grep -p $ID results/${file_name}_hits_1e-5_kingdom=all.faa | seqkit subseq -r $s:$e -R; done < <(cat results/${file_name}_coord_kingdom=all.tsv | grep -v "#" | awk -v min_q_coverage="$min_q_cov" '$5 > min_q_coverage') > results/${file_name}_hit-region_1e-5_qcov-gt-${min_q_cov}_kingdom=all.faa
